...
export function deepReplaceMatch(obj, testVal, testProp, cb, id) {
...
export async function getRawData(modPath){
    ...
    let ModData = {
        includes: (()=>{
            let includes = [
                ...
                "waterdata.xml",
                "weapondata.xml",
            ].map(el => "Base.SC2Data/GameData/" + el);
    ...
    for(let includedFile of ModData.includes){
        ...
        if(!data){
            ...
            ModData.catalogs[includedFile] = data.Catalog?.constructor === Object ? data : {}
        }
    }
...
export async function getGameData({mods, data, structure}){
    ...
    for(let mod of mods){
        ...
        for(let i in rawData.catalogs){
            AssignData(modData, rawData.catalogs[i].Catalog,structure)
        }
...
export function resolveImagesTags(gameData){
    deepReplaceMatch(gameData, null, prop => prop === "icon" || prop === "wireframe", ({val, obj, prop, id}) => {
        ...
        if(!gameData.icons.includes(obj[prop])){
...
export async function combineMods({input,output}){
            ...
            if(!field.startsWith("?") && Instance[field].constructor === Array){
                ...
                for(let Entity of Instance[field]) {
                    ...
                    if(result[field].constructor === String){
                        console.log("potential data problem " , result, field)
    ...
    for(let mod of mods){
        ...
        for(let catalogID in  modData.catalogs){
            let catalogs = modData.catalogs[catalogID].Catalog
            if(catalogs){
    ...
    let resultData = {}
    let catalogs = "abil,accumulator,achievement,achievementterm,actor,actorsupport,alert,armycategory,armyunit,armyupgrade,artifact,artifactslot,attachmethod,bankcondition,beam,behavior,boost,bundle,button,camera,campaign,character,cliff,cliffmesh,collection,colorstyle,commander,config,consoleskin,conversation,conversationstate,cursor,datacollection,datacollectionpattern,decalpack,dsp,effect,emoticon,emoticonpack,error,footprint,fow,game,gameui,herd,herdnode,heroabil,hero,herostat,itemclass,itemcontainer,item,kinetic,lensflareset,light,loot,map,model,mount,mover,objective,physicsmaterial,ping,playerresponse,portraitpack,race,requirement,requirementnode,reverb,reward,scoreresult,scorevalue,shape,skin,skinpack,sound,soundexclusivity,soundmixsnapshot,soundtrack,spray,spraypack,stimpack,taccooldown,tactical,talent,talentprofile,targetfind,targetsort,texture,texturesheet,turret,unit,upgrade,user,validator,voiceover,voicepack,weapon"
        .split(",")
